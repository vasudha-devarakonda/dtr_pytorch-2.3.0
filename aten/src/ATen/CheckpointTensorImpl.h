...
// The tensor value might be dropped, and when requested later, recomputed and
// cached again.
...
//   To do this, the wrapper must be require_grad, and the wrapped value must
//   not.
// A CheckpointedTensor might be constant.
//   In this case it is unevictable.
// An operator might return multiple output.
//   In this case the computation info (rematerializer) is shared between all of
//   them, And when the function get computed again all value get cached.
...
//   we make a rematerialize_function that take in the base tensor (other tensor
//   alias from) and output all the new value of the aliases, then update the
//   Ref. Of course, the cleaner way is to not support this. Shame on those who
//   use this feature.
...
// This ensure that everything will be released if there is no external ref
// whatsoever.

// Optimization:
// We treat tensor that has no external reference differently -
// They will never be externally used again so we assume their next use time is
// infinite so, if it doesnt has any evicted neighbor it will get evicted
// immediately.

// Note: to code fast I do not use RAII and just assume the code will not try to
// recover from exception. It should be easy to fix though.

namespace at {

// TODO: using a pool allocator might make more sense - no need to allocate and
...
struct CheckpointInfo {
  ...
  CheckpointInfo(duration_t compute_cost) : compute_cost(compute_cost) {}
};

// ecn represent a evicted tensor group.
// it is a set of tensor that are evicted, and if two evicted tensor are input
// -> output to each other, they must be in an ecn. note: we try to support
// removal from ecn by subtracting compute_cost and memory. this will create
// suprious connection but that should be fine empircally. below is an example
// of a suprious connection: a -> b, a -> c a, b, c got evicted so belong to a
// single ecn. a got rematerialized. b, c still belong to a single ecn although
...
struct Rematerializer : intrusive_ptr_target {
  ...
  Rematerializer(const Unsafe&,
                 ...
                 duration_t compute_cost,
                 std::string name)
      : func(func), inputs(inputs), compute_cost(compute_cost), name(name) {}
  void release_resources() final {
...
// This is the atomic level of eviction - when evicting, everything here will
// get evicted. When an AliasPool is evicted, the Storage of the underlying
// tensor must be freed. Additionally, the AliasPool contain weak pointer to all
// children of tensors, in order to compute the score of evicting a Storage.
struct AliasPool : intrusive_ptr_target {
  ...
  // it is likely that get() will run out of memory, and when it does so, it
  // will try to evict. so, it is crucial that we dont try to evict those
  // tensors - doing so will not evict anything. lock_count count how many time
  // a tensor is referenced by get.
  size_t lock_count = 0;
  ...
  // An aliaspool cant register itself to the checkpointpool - you have to do it
  // yourself.
  AliasPool(const Unsafe&, intrusive_ptr<Rematerializer> head_remat, size_t memory)
      : head_remat(head_remat), memory(memory), last_used_time(std::chrono::system_clock::now()) {}
  ...
  // have to check so, because when we rematerialize a single tensor in an
  // aliaspool, we will set it to non-evicted, and when we rematerialize it's
  // tensor they will also reset this.
...
struct CheckpointTensorCell : intrusive_ptr_target {
  ...
  ScalarType scalar_type() const {
    // std::cout << "ppppppp getting the scalar from checkpoint" << typeMetaToScalarType(dtype()) << "\n";
    return typeMetaToScalarType(dtype());
  }
  ...
  void evict() {
    // std::cout << "evicted tensor size "<< memory() << " will be recomputed using " << remat->name << "\n";
    TORCH_CHECK(remat);
    t.reset();
  }
  void fill(const Tensor& t);
  explicit CheckpointTensorCell(const Tensor& t, const intrusive_ptr<AliasPool>& pool) : pool(pool) {
    // std::cout << "tensor cell struct\n";
    // std::cout << "tensor" << t.numel() << "\n";
    fill(t);
    // std::cout << "tensor" << t.numel() << "\n";
  }
  explicit CheckpointTensorCell(const Tensor& t,
    ...
    // std::cout << "tensor cell struct with remat\n";
    fill(t);
  }
  size_t memory() {
    TORCH_CHECK(defined);
  ...
  Device device() const {
    return Device(c10::DeviceType::CUDA, 0);
  }
  Tensor get() {
    // std::cout << "getting the value ";
    if (!t) {
      TORCH_CHECK(remat);
      // std::cout << "recomputation \n";
      remat->remat();
    }
    TORCH_CHECK(t);
    // std::cout << "check diapcth key\n";
    TORCH_CHECK(!t->key_set().has(DispatchKey::Checkpoint));
    // std::cout << "check diapcth key no\n";
    // std::cout << "Tensor dispatch keys: " << t->key_set() << std::endl;
...
// Thus, when it die we can apply optimization like banishing/infinite
// staleness. We keep this invariant by only allowing CheckpointTensorImpl to
// make new External, When new CheckpointTensorImpl is constructed.
struct External : intrusive_ptr_target {
  ...
  External(const Tensor& value)
      : External(
            strong::make(value,
                         intrusive_ptr<AliasPool>::make(Unsafe(), intrusive_ptr<Rematerializer>(), memory(value)))) {}
  External(const Tensor& value, const intrusive_ptr<AliasPool>& pool, const intrusive_ptr<Rematerializer>& remat)
...
struct CheckpointTensorImpl : TensorImpl {
  ...
  static Tensors make(const std::string& name, const rematerialize_function_t& remat, const Tensors& inputs);
  ...
  void test() const override {
    std::cout << "tetsing the virtua from chekpooint\n";
  }
  ...
  bool has_storage() const override {
    // std::cout << "-----has storage----\n";
    return true;
  }
  c10::SymIntArrayRef sym_strides() const override {
    // std::cout << "sys strides checkpoint\n";
    return ref->value->value->get().sym_strides();
  }
  c10::SymInt sym_numel() const override {
    // std::cout << "sys strides numel\n";
    return ref->value->value->get().sym_numel();
  }
  SymIntArrayRef sym_sizes() const override {
    // std::cout << "sys strides sizes\n";
    return ref->value->value->get().sym_sizes();
  }
  const caffe2::TypeMeta dtype() const override {
    // std::cout << "calling the data from checkpoint implementation ---- \n";
    return ref->value->value->get().dtype();
  }
};

// CheckpointPool keep a list of AliasPool, and search over them to choose the
// best one to evict.
