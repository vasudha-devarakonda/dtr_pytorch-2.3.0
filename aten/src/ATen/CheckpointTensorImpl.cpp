...
struct PerfStats {
  ...
  ~PerfStats() {
    ...
    std::sort(
        data.begin(),
        data.end(),
        [](const std::pair<std::string, Duration>& a,
           const std::pair<std::string, Duration>& b) -> bool {
    ...
    for (auto d : data) {
      ...
      double percentage =
          ((double)duration.count()) / ((double)total_duration.count()) * 100;
      auto call_count = this->calls.find(d.first);
      TORCH_CHECK(call_count != this->calls.end());
      std::cout << "CallSite: " << d.first
                << " CallCount: " << call_count->second
                << " Cost: " << duration.count() << "ns" << " (%" << percentage
                << ")" << std::endl;
    }
    // std::cout << "Tracked events:\n";
...
void CheckpointPool::add(const intrusive_ptr<AliasPool>& p) {
  if (p->memory > 0 &&
      (memory_count == 0 || !ignore_small_tensors ||
       p->memory >= 0.01 * double(memory_sum / memory_count))) {
...
void CheckpointPool::evict() {
  ...
  std::uniform_int_distribution<> distrib(
      1, 1 * std::max(1, static_cast<int>(std::sqrt(aps.size()))));
  // sampling a random independent subset of all evictable tensors to find the
  // cheapest tensor to evict.
...
c10::string_view repr(const Tensor& t) {
  std::stringstream ss;
  ss << "CheckpointTensorImpl(size=" << t.sizes() << ", numel=" << t.numel()
     << ")";
  return ss.str();
}

Tensor print_custom(const Tensor& t) {
  return t;
}
Tensor checkpoint(const Tensor& t) {
  bool req_grad = t.requires_grad();
  STATS.track("checkpoint");
  // std::cout << "Cehckpoint\n";
  auto cpti = intrusive_ptr<CheckpointTensorImpl>::make(t);
  if (req_grad) {
    Tensor(cpti).set_requires_grad(true);
  }
  if (use_log_) {
    DTRLogConstant(cpti->counter_name());
    DTRLogMemory(cpti->counter_name(), cpti->ref->value->value->memory());
  }
  Tensor ret = Tensor(cpti);
  // std::cout << " dim " << ret.dim() << "\n";
  return Tensor(cpti);
}
Tensor uncheckpoint(const Tensor& t) {
  ...
  // std::cout << "uncheckpoint before running the operator\n";
...
Tensor try_checkpoint(const Tensor& t) {
  ...
  // std::cout << t.device() <<"try checkpoint single input\n";
  return is_checkpoint(t) ? t : checkpoint(t);
}
void new_log(std::string str) {
  DTRLogger::logger().out =
      std::ofstream(DTRLogger::logger().get_filename(str));
}
...
[[inline]] Tensor uncheckpoint(const strong& input) {
  return input->get();
}
...
Tensors uncheckpoint(const strongs& inputs) {
  STATS.track("uncheckpoint");
  Tensors ret;
  // std::cout << "uncheckpoint multiple inputs before running the operator\n";
...
void AliasPool::set_not_evicted(const intrusive_ptr<AliasPool>& self) {
  if (unlikely(is_evicted)) {
    ...
    if (ecn) {
      ...
      auto cpi = get_t(ecn);
      update_t(
          ecn, CheckpointInfo(cpi.compute_cost - head_remat->compute_cost));
      ecn.reset();
    }
    pool.add(self);
  }
...
void CheckpointTensorCell::fill(const Tensor& t) {
  ...
  // std::cout << "fill checkpoint\n";
  // std::cout << "tensor1: " << t.numel() << "\n";
  // std::cout << "checking if the grad is required " << t.requires_grad() <<
  // "\n";
  if (!(this->t)) {
...
intrusive_ptr<TensorImpl> CheckpointTensorImpl::shallow_copy_and_detach(
    const VariableVersion& version_counter,
    bool allow_tensor_metadata_change) const {
  // std::cout << "sallow copy and detach checkpoint\n";
...
void CheckpointTensorImpl::shallow_copy_from(
...
// while there are two vector, one storing nonconstants and one storing
// constants. the constants are small and they will not be considered for
// eviction. however, we have to stitch the two vectors together to pass it in
// remat. the size_t in constants decide the location to stitch them in, while
// input_values fill in the rest.
MakeRawResult make_raw(
  ...
  for (const Tensor& t : raw_outputs) {
    if (t.defined()) {
      ...
      if (alias == -1) {
        auto m = memory(t);
        // std::cout << "the memory of the output for " << name_of_function << "
        // is" << m << "\n";
  ...
  return {outputs, aliases, post - pre, remat, raw_outputs.size()};
}
...
Tensors CheckpointTensorImpl::make(
    const std::string& name,
    const rematerialize_function_t& remat,
    const Tensors& inputs) {
  STATS.track("CheckPointTensorImpl::make");
  // std::cout << "the size of inputs is " << "\n";
  Tensors checkpointed_inputs = try_checkpoint(inputs);
  // this contains all the inputs of checkpointed
  ...
  auto ret = make_raw(name, remat, input_values);
  // std::cout << "the size of ret outputs" << ret.outputs.size() << "\n";
  Tensors tensors;

  tensors.reserve(ret.raw_output_size);
  for (const auto& t : ret.outputs) {
    auto cp = Tensor(intrusive_ptr<CheckpointTensorImpl>::make(t));
    tensors.push_back(cp);
  }
  // std::cout << "the outputs are made===\n";
  if (ret.outputs.size() != ret.raw_output_size) {
    int64_t num_empty = ret.raw_output_size - ret.outputs.size();
    while(num_empty > 0) {
...
void CheckpointTensorImpl::mutate(
    const std::string& name,
